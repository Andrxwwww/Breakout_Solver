...
public class BreakoutGeneticAlgorithm {
    ...
    private void initializePopulation() {
        for (int i = 0; i < POPULATION_SIZE; i++) {
            population[i] = new NeuralNetwork(Commons.BREAKOUT_STATE_SIZE, Commons.BREAKOUT_NUM_ACTIONS);
        }
    }
    ...
    public NeuralNetwork evolve(int seed) {
        ...
        // Return the best neural network from the final population
    ...
    private NeuralNetwork crossover(NeuralNetwork parent1, NeuralNetwork parent2) {
        NeuralNetwork offspring = new NeuralNetwork(Commons.BREAKOUT_STATE_SIZE, Commons.BREAKOUT_NUM_ACTIONS);
    
        int crossoverPoint1 = random.nextInt(Commons.BREAKOUT_STATE_SIZE);
        int crossoverPoint2 = random.nextInt(Commons.BREAKOUT_STATE_SIZE);
    
        for (int i = 0; i < Commons.BREAKOUT_STATE_SIZE; i++) {
            for (int j = 0; j < 16; j++) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < Commons.BREAKOUT_NUM_ACTIONS; j++) {
                if (i < crossoverPoint1 || i > crossoverPoint2) {
    ...
    private double evaluateFitness(NeuralNetwork network , int seed) {
        ...
        BreakoutBoard breakoutBoard = new BreakoutBoard(nnGameController, false, seed);
